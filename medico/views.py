...
def finalizar_consulta(request, id_consulta):
    ...
    consulta = Consulta.objects.get(id=id_consulta)
    # TODO: fazer essa validação em outras areas necessitadas dela
    if request.user == consulta.data_aberta.user:
        add_message(request, constants.ERROR, 'Essa consulta não pertence ao medico atual.')
        return redirect(f'/medicos/abrir_horario')
    consulta.status = 'F'
    consulta.save()
    return redirect(f'/medicos/consulta_area_medico/{id_consulta}')

    # TODO: fazer botão de cancelar consulta no paciente
...
def add_documento(request, id_consulta):
    ...
    if not documento:
        add_message(request, constants.ERROR, 'Campo de documento não preenchido.')
        return redirect(f'/medicos/consulta_area_medico/{id_consulta}')
